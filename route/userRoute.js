...
route.get('/data/get', auth, async (req, res) => {
  try {
    const { roles, id } = req.user; // Extract roles and user ID
    ...
    if (!page || !limit) {
      ...
      if (roles === "Admin" || roles === "HR") {
        ...
        const employees = await User.find({ supervisor: id, roles: "Employee" })
          .populate("assigned", "projectName projectId")
          .populate("leave", "totalLeaves")
          .populate("attendance")
          .sort({ _id: -1 });

        const clients = await User.find({ roles: "Client", supervisor: id })
          .populate("assigned", "projectName projectId")
          .sort({ _id: -1 });
        ...
        const employees = await User.find({ manager: id, roles: "Employee" })
          .populate("assigned", "projectName projectId")
          .populate("leave", "totalLeaves")
          .populate("attendance")
          .sort({ _id: -1 });

        const clients = await User.find({ roles: "Client", manager: id })
          .populate("assigned", "projectName projectId")
          .sort({ _id: -1 });
      ...
      return res.status(200).json({
        ...
        totalEmployees,
        totalClients,
        pagination: false, // Indicate no pagination applied
      });
    }
    ...
    if (roles === "Admin" || roles === "HR") {
      ...
      users = { employees, clients };
      totalEmployees = await User.countDocuments({ supervisor: id, roles: "Employee" });
      totalClients = await User.countDocuments({ roles: "Client", supervisor: id });
      ...
      users = { employees, clients };
      totalEmployees = await User.countDocuments({ manager: id, roles: "Employee" });
      totalClients = await User.countDocuments({ roles: "Client", manager: id });
    ...
    res.status(200).json({
      ...
      totalEmployees,
      totalClients,
      totalPagesEmployees: Math.ceil(totalEmployees / limit),
      totalPagesClients: Math.ceil(totalClients / limit),
      currentPage: parseInt(page),
      perPage: parseInt(limit),
      pagination: true, // Indicate pagination applied
    });
...
route.get("/client", auth, async (req, res) => {
    try {
